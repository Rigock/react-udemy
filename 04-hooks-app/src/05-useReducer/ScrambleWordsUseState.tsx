...
export const ScrambleWords = () => {
  ...
  const handleGuessSubmit = (e: React.FormEvent) => {
    ...
    if (errorCounter === 2){
      ...
      // setWords([])  //FIN DEL JUEGO
  };

  const handleSkip = () => {
    const index = words.findIndex(i => i === currentWord);
    if(index !== -1) {
      let newWords = [...words];
      console.log({newWords});
      const [wordToSkip] = newWords.splice(index, 1);
      newWords = [...newWords, wordToSkip];
      console.log({newWords});
      setWords(newWords);
      setCurrentWord(newWords[0]);
      setScrambledWord(scrambleWord(newWords[0]));
    ...
    // if(skipCounter === 3) setMaxSkips(0);
  };

  const handlePlayAgain = () => {
    const newGameWords = shuffleArray(GAME_WORDS);

    setWords(newGameWords);
    setCurrentWord(newGameWords[0]);
    setScrambledWord(newGameWords[0]);
    setGuess('');
    setErrorCounter(0)
    setIsGameOver(false);
    setPoints(0);
